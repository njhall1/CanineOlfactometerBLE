...
BLECharacteristic OdorValveCharacteristic2("19B10003-E8F2-537E-4F6C-D104768A1214", BLERead | BLEWriteWithoutResponse,14);
...
float sniff;
char newSniffCrit[8];
char newOdorActivation[14];
//char pinValue[8];
int myPin;
char odorstatus;
long odorActivation=0;
int myPinArray[14]={A3, A2, A1, A0, 13, 12, 2, 3, 4, 5, 6, 7, 8,9};
...
void setup() {
  ...
  /////////////////////////////////////////////////
  BLE.setLocalName("Olfactometer1");
  ////////////////////////////////////////////////
...
//  pinMode(RED_PIN, OUTPUT); 
  ...
  OdorService.addCharacteristic(OdorValveCharacteristic2);
  ...
  if (central) {
    ...
    while (central.connected()) {
      ...
      if (OdorSwitchCharacteristic.written()) {
          ...
          else if (ActDeactivate=='3'){
            OdorValveCharacteristic2.readValue(newOdorActivation, 14);

            for (int i=0;i<14;i++){
              odorstatus=newOdorActivation[i];
              pin=myPinArray[i];
